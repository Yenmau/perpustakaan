@if (Auth::check())
    ...
    @section('content2')
        <div class="py-[80px]">
            <form method="POST" action="{{ route('pinjam.update', $pinjam->kode_pinjam) }}"
                ...
                <div class="grid grid-cols-2 gap-4 py-[10px]">
                    <div class="mb-1">
                        ...
                        <input type="text" name="kode_pinjam" id="kode_pinjam"
                            ...
                            class="w-full border-2 border-red-600 p-2 rounded-md">
                    </div>
                        ...
                        <select name="nomor_anggota" id="nomor_anggota"
                            ...
                            @foreach ($anggota as $a)
                                <option value="{{ $a->nomor_anggota }}"
                                    {{ old('nomor_anggota', $pinjam->nomor_anggota) == $a->nomor_anggota ? 'selected' : '' }}>
                                    {{ $a->nama_anggota }}
                                </option>
                            @endforeach
                        </select>
